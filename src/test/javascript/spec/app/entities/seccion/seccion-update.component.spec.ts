/* tslint:disable max-line-length */
import { shallowMount, createLocalVue, Wrapper } from '@vue/test-utils';
import sinon, { SinonStubbedInstance } from 'sinon';
import Router from 'vue-router';
import { ToastPlugin } from 'bootstrap-vue';

import dayjs from 'dayjs';
import { DATE_TIME_LONG_FORMAT } from '@/shared/date/filters';

import * as config from '@/shared/config/config';
import SeccionUpdateComponent from '@/entities/seccion/seccion-update.vue';
import SeccionClass from '@/entities/seccion/seccion-update.component';
import SeccionService from '@/entities/seccion/seccion.service';

import UbicacionInventarioService from '@/entities/ubicacion-inventario/ubicacion-inventario.service';

import NivelService from '@/entities/nivel/nivel.service';
import AlertService from '@/shared/alert/alert.service';

const localVue = createLocalVue();

config.initVueApp(localVue);
const i18n = config.initI18N(localVue);
const store = config.initVueXStore(localVue);
const router = new Router();
localVue.use(Router);
localVue.use(ToastPlugin);
localVue.component('font-awesome-icon', {});
localVue.component('b-input-group', {});
localVue.component('b-input-group-prepend', {});
localVue.component('b-form-datepicker', {});
localVue.component('b-form-input', {});

describe('Component Tests', () => {
  describe('Seccion Management Update Component', () => {
    let wrapper: Wrapper<SeccionClass>;
    let comp: SeccionClass;
    let seccionServiceStub: SinonStubbedInstance<SeccionService>;

    beforeEach(() => {
      seccionServiceStub = sinon.createStubInstance<SeccionService>(SeccionService);

      wrapper = shallowMount<SeccionClass>(SeccionUpdateComponent, {
        store,
        i18n,
        localVue,
        router,
        provide: {
          seccionService: () => seccionServiceStub,
          alertService: () => new AlertService(),

          ubicacionInventarioService: () =>
            sinon.createStubInstance<UbicacionInventarioService>(UbicacionInventarioService, {
              retrieve: sinon.stub().resolves({}),
            } as any),

          nivelService: () =>
            sinon.createStubInstance<NivelService>(NivelService, {
              retrieve: sinon.stub().resolves({}),
            } as any),
        },
      });
      comp = wrapper.vm;
    });

    describe('load', () => {
      it('Should convert date from string', () => {
        // GIVEN
        const date = new Date('2019-10-15T11:42:02Z');

        // WHEN
        const convertedDate = comp.convertDateTimeFromServer(date);

        // THEN
        expect(convertedDate).toEqual(dayjs(date).format(DATE_TIME_LONG_FORMAT));
      });

      it('Should not convert date if date is not present', () => {
        expect(comp.convertDateTimeFromServer(null)).toBeNull();
      });
    });

    describe('save', () => {
      it('Should call update service on save for existing entity', async () => {
        // GIVEN
        const entity = { id: 123 };
        comp.seccion = entity;
        seccionServiceStub.update.resolves(entity);

        // WHEN
        comp.save();
        await comp.$nextTick();

        // THEN
        expect(seccionServiceStub.update.calledWith(entity)).toBeTruthy();
        expect(comp.isSaving).toEqual(false);
      });

      it('Should call create service on save for new entity', async () => {
        // GIVEN
        const entity = {};
        comp.seccion = entity;
        seccionServiceStub.create.resolves(entity);

        // WHEN
        comp.save();
        await comp.$nextTick();

        // THEN
        expect(seccionServiceStub.create.calledWith(entity)).toBeTruthy();
        expect(comp.isSaving).toEqual(false);
      });
    });

    describe('Before route enter', () => {
      it('Should retrieve data', async () => {
        // GIVEN
        const foundSeccion = { id: 123 };
        seccionServiceStub.find.resolves(foundSeccion);
        seccionServiceStub.retrieve.resolves([foundSeccion]);

        // WHEN
        comp.beforeRouteEnter({ params: { seccionId: 123 } }, null, cb => cb(comp));
        await comp.$nextTick();

        // THEN
        expect(comp.seccion).toBe(foundSeccion);
      });
    });

    describe('Previous state', () => {
      it('Should go previous state', async () => {
        comp.previousState();
        await comp.$nextTick();

        expect(comp.$router.currentRoute.fullPath).toContain('/');
      });
    });
  });
});
