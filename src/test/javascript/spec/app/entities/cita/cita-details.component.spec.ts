/* tslint:disable max-line-length */
import { shallowMount, createLocalVue, Wrapper } from '@vue/test-utils';
import sinon, { SinonStubbedInstance } from 'sinon';
import VueRouter from 'vue-router';

import * as config from '@/shared/config/config';
import CitaDetailComponent from '@/entities/cita/cita-details.vue';
import CitaClass from '@/entities/cita/cita-details.component';
import CitaService from '@/entities/cita/cita.service';
import router from '@/router';
import AlertService from '@/shared/alert/alert.service';

const localVue = createLocalVue();
localVue.use(VueRouter);

config.initVueApp(localVue);
const i18n = config.initI18N(localVue);
const store = config.initVueXStore(localVue);
localVue.component('font-awesome-icon', {});
localVue.component('router-link', {});

describe('Component Tests', () => {
  describe('Cita Management Detail Component', () => {
    let wrapper: Wrapper<CitaClass>;
    let comp: CitaClass;
    let citaServiceStub: SinonStubbedInstance<CitaService>;

    beforeEach(() => {
      citaServiceStub = sinon.createStubInstance<CitaService>(CitaService);

      wrapper = shallowMount<CitaClass>(CitaDetailComponent, {
        store,
        i18n,
        localVue,
        router,
        provide: { citaService: () => citaServiceStub, alertService: () => new AlertService() },
      });
      comp = wrapper.vm;
    });

    describe('OnInit', () => {
      it('Should call load all on init', async () => {
        // GIVEN
        const foundCita = { id: 123 };
        citaServiceStub.find.resolves(foundCita);

        // WHEN
        comp.retrieveCita(123);
        await comp.$nextTick();

        // THEN
        expect(comp.cita).toBe(foundCita);
      });
    });

    describe('Before route enter', () => {
      it('Should retrieve data', async () => {
        // GIVEN
        const foundCita = { id: 123 };
        citaServiceStub.find.resolves(foundCita);

        // WHEN
        comp.beforeRouteEnter({ params: { citaId: 123 } }, null, cb => cb(comp));
        await comp.$nextTick();

        // THEN
        expect(comp.cita).toBe(foundCita);
      });
    });

    describe('Previous state', () => {
      it('Should go previous state', async () => {
        comp.previousState();
        await comp.$nextTick();

        expect(comp.$router.currentRoute.fullPath).toContain('/');
      });
    });
  });
});
