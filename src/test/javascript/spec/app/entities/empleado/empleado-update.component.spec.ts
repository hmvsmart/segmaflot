/* tslint:disable max-line-length */
import { shallowMount, createLocalVue, Wrapper } from '@vue/test-utils';
import sinon, { SinonStubbedInstance } from 'sinon';
import Router from 'vue-router';
import { ToastPlugin } from 'bootstrap-vue';

import dayjs from 'dayjs';
import { DATE_TIME_LONG_FORMAT } from '@/shared/date/filters';

import * as config from '@/shared/config/config';
import EmpleadoUpdateComponent from '@/entities/empleado/empleado-update.vue';
import EmpleadoClass from '@/entities/empleado/empleado-update.component';
import EmpleadoService from '@/entities/empleado/empleado.service';

import EmpleadoTipoVehiculoService from '@/entities/empleado-tipo-vehiculo/empleado-tipo-vehiculo.service';

import ExperienciaHabilidadService from '@/entities/experiencia-habilidad/experiencia-habilidad.service';

import LicenciaManejoService from '@/entities/licencia-manejo/licencia-manejo.service';

import HorarioService from '@/entities/horario/horario.service';

import OperadorUnidadService from '@/entities/operador-unidad/operador-unidad.service';

import PersonaService from '@/entities/persona/persona.service';

import SucursalService from '@/entities/sucursal/sucursal.service';
import AlertService from '@/shared/alert/alert.service';

const localVue = createLocalVue();

config.initVueApp(localVue);
const i18n = config.initI18N(localVue);
const store = config.initVueXStore(localVue);
const router = new Router();
localVue.use(Router);
localVue.use(ToastPlugin);
localVue.component('font-awesome-icon', {});
localVue.component('b-input-group', {});
localVue.component('b-input-group-prepend', {});
localVue.component('b-form-datepicker', {});
localVue.component('b-form-input', {});

describe('Component Tests', () => {
  describe('Empleado Management Update Component', () => {
    let wrapper: Wrapper<EmpleadoClass>;
    let comp: EmpleadoClass;
    let empleadoServiceStub: SinonStubbedInstance<EmpleadoService>;

    beforeEach(() => {
      empleadoServiceStub = sinon.createStubInstance<EmpleadoService>(EmpleadoService);

      wrapper = shallowMount<EmpleadoClass>(EmpleadoUpdateComponent, {
        store,
        i18n,
        localVue,
        router,
        provide: {
          empleadoService: () => empleadoServiceStub,
          alertService: () => new AlertService(),

          empleadoTipoVehiculoService: () =>
            sinon.createStubInstance<EmpleadoTipoVehiculoService>(EmpleadoTipoVehiculoService, {
              retrieve: sinon.stub().resolves({}),
            } as any),

          experienciaHabilidadService: () =>
            sinon.createStubInstance<ExperienciaHabilidadService>(ExperienciaHabilidadService, {
              retrieve: sinon.stub().resolves({}),
            } as any),

          licenciaManejoService: () =>
            sinon.createStubInstance<LicenciaManejoService>(LicenciaManejoService, {
              retrieve: sinon.stub().resolves({}),
            } as any),

          horarioService: () =>
            sinon.createStubInstance<HorarioService>(HorarioService, {
              retrieve: sinon.stub().resolves({}),
            } as any),

          operadorUnidadService: () =>
            sinon.createStubInstance<OperadorUnidadService>(OperadorUnidadService, {
              retrieve: sinon.stub().resolves({}),
            } as any),

          personaService: () =>
            sinon.createStubInstance<PersonaService>(PersonaService, {
              retrieve: sinon.stub().resolves({}),
            } as any),

          sucursalService: () =>
            sinon.createStubInstance<SucursalService>(SucursalService, {
              retrieve: sinon.stub().resolves({}),
            } as any),
        },
      });
      comp = wrapper.vm;
    });

    describe('load', () => {
      it('Should convert date from string', () => {
        // GIVEN
        const date = new Date('2019-10-15T11:42:02Z');

        // WHEN
        const convertedDate = comp.convertDateTimeFromServer(date);

        // THEN
        expect(convertedDate).toEqual(dayjs(date).format(DATE_TIME_LONG_FORMAT));
      });

      it('Should not convert date if date is not present', () => {
        expect(comp.convertDateTimeFromServer(null)).toBeNull();
      });
    });

    describe('save', () => {
      it('Should call update service on save for existing entity', async () => {
        // GIVEN
        const entity = { id: 123 };
        comp.empleado = entity;
        empleadoServiceStub.update.resolves(entity);

        // WHEN
        comp.save();
        await comp.$nextTick();

        // THEN
        expect(empleadoServiceStub.update.calledWith(entity)).toBeTruthy();
        expect(comp.isSaving).toEqual(false);
      });

      it('Should call create service on save for new entity', async () => {
        // GIVEN
        const entity = {};
        comp.empleado = entity;
        empleadoServiceStub.create.resolves(entity);

        // WHEN
        comp.save();
        await comp.$nextTick();

        // THEN
        expect(empleadoServiceStub.create.calledWith(entity)).toBeTruthy();
        expect(comp.isSaving).toEqual(false);
      });
    });

    describe('Before route enter', () => {
      it('Should retrieve data', async () => {
        // GIVEN
        const foundEmpleado = { id: 123 };
        empleadoServiceStub.find.resolves(foundEmpleado);
        empleadoServiceStub.retrieve.resolves([foundEmpleado]);

        // WHEN
        comp.beforeRouteEnter({ params: { empleadoId: 123 } }, null, cb => cb(comp));
        await comp.$nextTick();

        // THEN
        expect(comp.empleado).toBe(foundEmpleado);
      });
    });

    describe('Previous state', () => {
      it('Should go previous state', async () => {
        comp.previousState();
        await comp.$nextTick();

        expect(comp.$router.currentRoute.fullPath).toContain('/');
      });
    });
  });
});
