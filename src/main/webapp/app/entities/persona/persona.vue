<template>
  <div>
    <h2 id="page-heading" data-cy="PersonaHeading">
      <span v-text="$t('segmaflotApp.persona.home.title')" id="persona-heading">Personas</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('segmaflotApp.persona.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'PersonaCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('segmaflotApp.persona.home.createLabel')"> Create a new Persona </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && personas && personas.length === 0">
      <span v-text="$t('segmaflotApp.persona.home.notFound')">No personas found</span>
    </div>
          ...
          <tr v-for="persona in personas" :key="persona.id" data-cy="entityTable">
            ...
            <td class="text-right">
              <div class="btn-group">
                ...
                <b-button
                  v-on:click="prepareRemove(persona)"
                  variant="danger"
                  class="btn btn-sm"
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="segmaflotApp.persona.delete.question" data-cy="personaDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-persona-heading" v-text="$t('segmaflotApp.persona.delete.question', { id: removeId })">
